...
| **Full-article cost, measured, not projected** | A 4,893-word real corpus article (`tests/corpus`, "A Drug-Trial Stock Sale"), 59 blocks, 31.0 minutes of audio, on this box: build (4 `kokoro-onnx` instances) 21.4 min, RTF 0.691; align 13.2 min, RTF 0.425 — matching the RTF measured on short synthetic clips, so that number generalises. Align added 62% to the total time this article took to fully process (21.4 → 34.6 min). The build-phase RTF is higher here than the 0.456 in "The engines" table above because this run overlapped a real rebuild on the live site sharing the same four cores — a confound worth re-measuring clean, not a change in the engine's own cost. Peak child RSS during the align phase (2.58 GB) was well above the 570–830 MB the short-clip spike suggested; the aligner does no `gc.collect()`/`malloc_trim(0)` between blocks the way the build worker's own engine pool release already does, and 59 sequential alignments' allocator arenas likely never gave memory back — worth fixing before this is trusted at scale, not yet done. All 59 blocks aligned successfully. |
| **`word_highlight` defaults off, and has no UI toggle yet** | The measured cost above is a real, not-obviously-worth-it trade for every article regardless of whether anyone looks at the word highlight — a decision for whoever runs the build, not a default to flip silently. The only way to turn it on today is a build option passed programmatically; the Voice/settings page has no checkbox for it. |
