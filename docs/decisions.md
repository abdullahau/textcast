...
| Decision | Evidence |
| --- | --- |
| **The aligner is `wav2vec2-base-960h` (Apache-2.0), against a hand-written numpy Viterbi, not `ctc-forced-aligner`'s own decoder** | Spiked first against that package's default, `mms-300m-1130-forced-aligner`: 2.45× real-time, 2.4–3.1 GB resident, CC-BY-NC licensed, and most of that cost turned out to be the package's own 30-second window padding a five-second clip up to encoding 34 seconds of near-silence. `wav2vec2-base-960h` int8 is a third the parameters, unrestricted licence, and — `window_length` sized to the clip instead of left at the default — RTF 0.19–0.29 on short clips with the package's compiled decoder. The compiled decoder needed `librosa`, `numba` and `scikit-learn` for two ideas (the windowing scheme, grouping a per-frame path into runs) that fit in under twenty lines each; a plain numpy Viterbi, verified to land within a few ms of the compiled reference on the same clips before being trusted, costs some CPU time and zero new dependencies — `onnxruntime`/`numpy`/`requests` already ship. |
| **`align` is its own job kind, its own lane** | The build worker's child process drains its *entire* queued backlog, keeping the engine pool warm across every article in it — releasing it between two phases of one article, the original plan, would have reloaded the model for the next article in the same drain. `align` joins `build` and `summarise` as a third lane instead: its own child process, so it never imports the TTS engine at all, and the two models are never resident together without fighting the pool's own reason to exist. |
| **Word timings live in two new `block` columns, not a re-read `manifest.json`** | The original plan had `align` read a build's own `manifest.json` back for `speech_start_ms`, the one field the database didn't store. Checking what actually consumed that file first turned up nothing at all — not the player, not the service worker, one test asserting it existed — so `render_article` stopped writing it, `speech_start_ms` became a column exactly like its `start_ms`/`dur_ms`/`speech_ms` siblings, and `sample_rate` (the file's other reason to exist) turned out to be a fixed per-engine constant, never data to begin with. |
| **Full-article cost, measured, not projected** | A 4,893-word real corpus article (`tests/corpus`, "A Drug-Trial Stock Sale"), 59 blocks, 31.0 minutes of audio, on this box: build (4 `kokoro-onnx` instances) 21.4 min, RTF 0.691; align, sequential, 13.2 min, RTF 0.425 — matching the RTF measured on short synthetic clips, so that number generalises. Sequential align added 62% to the total time this article took to fully process. The build-phase RTF is higher here than the 0.456 in "The engines" table above because this run overlapped a real rebuild on the live site sharing the same four cores — a confound worth re-measuring clean, not a change in the engine's own cost. Peak child RSS during the align phase (2.58 GB) was well above the 570–830 MB the short-clip spike suggested; the aligner does no `gc.collect()`/`malloc_trim(0)` between blocks the way the build worker's own engine pool release already does, and 59 sequential alignments' allocator arenas likely never gave memory back — worth fixing before this is trusted at scale, not yet done. All 59 blocks aligned successfully. |
| **Aligning blocks in parallel, one shared session, several threads — 3.5–3.7x** | Profiling one block found the Viterbi decode is 44 ms against 6.6 s of ONNX inference — 99% of the cost is inside `onnxruntime.InferenceSession.run()`, not this module's own Python. That call releases the GIL, the same reason `Run` is already documented thread-safe for the TTS engines (`docs/decisions.md`, "One session/model behind the whole pool"), so `align_article` gets a `concurrency` parameter and calls the *one* aligner from a `ThreadPoolExecutor` instead of building several instances the way the engine pool does. Measured twice: an isolated four-clips-at-once spike, 3.69x; the real full-article path on the same 59-block article above, sequential 13.2 min → parallel 3.72 min, 3.5x, all 59 blocks still aligned correctly. That drops the total-processing-time cost from 62% to roughly 17%. `intra_op_num_threads` stays at 1 regardless — a single call was already measured to gain nothing from extra intra-op threads, and letting it use more would have each of the concurrent calls compete for the same cores from underneath the caller's own parallelism. |
| **`word_highlight` defaults *on*; the article's own control is a `skip_word_highlight` checkbox at the end of the build options** | Reversed. It defaulted off on the reasoning below, which was written before any real article had been measured: at about a fifth more build time on a full article the read-along is cheap enough that having to ask for it was the wrong side to err on, and a feature nobody switches on is a feature nobody has. The per-article control keeps the shape of the three beside it -- checked writes an override the article keeps, unchecked defers to the Voice page -- so the only thing that changed is its sense and its position. One consequence, accepted: an article can now only turn highlighting *off* for itself. With the default off it could turn one article on, and nothing replaces that; if the default is ever switched back off, the per-article escape hatch has to come back with it. The original reasoning, kept because it is the measurement that was wrong and not the logic: Even at the ~17% parallel cost, it is still a real cost for every article regardless of whether anyone looks at the highlight — a decision for whoever runs the build, made deliberately, not a default flipped silently. Unchecking either box defers to the next layer down (article → site default), the same shape `skip_footnotes` and its siblings already use; `set_build_options` replaces the whole stored dict per save, so an unchecked box correctly clears a previous override rather than leaving it stuck on. Caught a real, unrelated bug adding the second checkbox: `save_voice_defaults` used to `return` early on an unparsable `speed` (an empty string, exactly what a blank form field sends), which silently skipped every parameter declared after it in the function — invisible for as long as `speed` was the last one. |
